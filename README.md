...
* `git clone <SSH> или <HTMl> ссылка` - Команда `git clone` автоматически связывает локальный и удалённый репозиторий.
...
## __Переключиться на другую ветку__
...
## __Сравниваем ветки__
* `git diff(tool - в случае использования P4Merge) <название_ветки1> <название_ветки2>` -  сравнить ветки
* `git diff(tool - в случае использования P4Merge) <название_ветки1> <хеш коммита>` -  сравнить ветку с хешем коммита

## __Суффикс навигации__
* `git diff(tool - в случае использования P4Merge) HEAD` - текущий коммит;
* `git diff(tool - в случае использования P4Merge) HEAD~` - следующий за текущим коммит;
* `git diff(tool - в случае использования P4Merge) HEAD~N` - где `N` — это число которое отсчитывает от заданного коммита `N` коммитов назад во времени.
